...
/*   Updated: 2023/02/27 20:17:03 by apena-ba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_doors(t_data *data, t_vec offset)
{
	...
	while (doors)
	{
		...
		door.edge = (t_edge){sub_vec(door.edge.start, offset),
			sub_vec(door.edge.end, offset), door.edge.tex_index};
...
void	draw_walls(t_data *data, t_vec offset)
{
	...
	while (points)
	{
		...
		draw_rectangle(data->minimap, sub_vec(vec_scl(point, TILE_SIZE),
				offset), (t_vec){TILE_SIZE, TILE_SIZE}, 0x4F4F4FFF);
		points = points->next;
	}
	...
	while (edges)
	{
		...
		edge = (t_edge){sub_vec(edge.start, offset), sub_vec(edge.end, offset),
			edge.tex_index};
...
void	draw_minimap(t_data *data)
{
	...
	offset = sub_vec(data->player_pos, (t_vec){MINIMAP_WIDTH / 2, MINIMAP_HEIGHT
			/ 2});
	draw_walls(data, offset);
	draw_circle(data->minimap, sub_vec(data->player_pos, offset),
		PLAYER_HITBOX_RADIUS * TILE_SIZE, 0x00FF00FF);
	draw_circle(data->minimap, sub_vec(data->player_pos, offset),
		PLAYER_INTERACTION_RADIUS * TILE_SIZE, 0x00FF00FF);
	draw_line(data->minimap, (t_edge){sub_vec(data->player_pos, offset),
		add_vec(sub_vec(data->player_pos, offset),
			vec_scl(calc_velocity(data, 0), 20)), 0}, 0xFFFFFFFF);
